...
User writes "Show air quality for Tokyo", names a place, and points to an empty area. Produce one html_widget there that uses that place in its API requests and displays the resulting air-quality information in large readable type.`,
  ...
  const I18N = {
    en: {
      ...
      themeFocusCanvasPro: "Favors professional whiteboard tools and right-side Lumi6 AI assistant",
      ...
      guideCanvasPro: "Lumi6 AI Teaching Assistant",
  ...
  function maybeStartOnboarding() {
    markChangelogSeen();
    if (document.getElementById("lumi6Onboard") && !document.getElementById("lumi6Onboard").hidden) return;
  ...
  function applyTheme(theme) {
    ...
    state.gridVisible = (localStorage.getItem("lumi6-grid") ?? localStorage.getItem("ghostboard-grid")) !== "false";
    updateThemeCopy();
    updateEmbodimentLabel();
    updateGridButton();
    updatePaint();
  ...
  function rememberRequest(id) {
    ...
    if (debugRequest) debugRequest.textContent = `request: ${id}`;
  }
  const key = (x, y) => `${x},${y}`;
...
// Canvas snapshots, export, drawing history, strokes, and lasso selection.
  ...
  function renderSidebarRecents() {
    const list = document.querySelector("#sidebarRecents");
    if (!list) return;
    list.replaceChildren();
    ...
    if (!snapshotItems.length) {
      ...
      empty.textContent = "New boards you save will show up here.";
      list.append(empty);
      return;
    }
    const location = state.snapshotLocation;
    for (const item of snapshotItems.slice(0, 24)) {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "sidebar-recent-item";
      button.textContent = snapshotName(item);
      button.title = snapshotName(item);
      if (item.id === state.currentSnapshotId && location === state.currentSnapshotLocation) {
        button.classList.add("current");
      }
      button.onclick = () => runSnapshotAction(() => loadSnapshot(item.id, location));
      list.append(button);
    }
  }
...
// AI requests, validation, generated drafts, plotting, and draft interaction.
  ...
  function restackFlowItems(items, gap = 72) {
    const flow = items
      .filter((item) => item?.command && ["write_text", "draw_formula"].includes(item.command.tool))
      .sort((a, b) => a.y - b.y || a.x - b.x);
    if (flow.length < 2) return;
    const columns = [];
    for (const item of flow) {
      const col = columns.find((entry) => Math.abs(entry.x - item.x) < 160);
    ...
    for (const col of columns) {
      ...
      for (const item of col.items) {
        const height = Math.max(1, item.image?.logicalHeight || item.layoutHeight || 200);
        item.y = y;
        item.layoutHeight = height;
        y += height + gap;
      }
      const overflow = y - gap - SIZE;
      if (overflow > 0) {
        for (const item of col.items) item.y = Math.max(0, item.y - overflow);
      }
    }
  }
  ...
  function drawPendingBatch(p, context = ctx) {
    ...
    for (const { item, index, box } of entries) {
      if (item.isTextBoxRecord) continue;
...
// Pointer and control bindings, portable snapshots, and application startup.
  ...
  document.querySelector("#penSize").oninput = (e) => {
    ...
    document.querySelector("#penSizeValue").textContent = `${state.pen}`;
  };
  ...
  function closeColorOrbs(except = null) {
    document.querySelectorAll("[data-color-control]").forEach((control) => {
      if (control === except || control.classList.contains("pen-color-control")) return;
  ...
  document.querySelectorAll("[data-color-control]").forEach((control) => {
    ...
    if (!control.classList.contains("pen-color-control")) {
  ...
  document.querySelectorAll(".orbit-swatch").forEach((button) => {
    ...
    const inPenTray = button.closest(".pen-color-control");
    button.setAttribute("tabindex", inPenTray ? "0" : "-1");
  ...
  document.querySelector("#exportPngBtn").onclick = () => {
  ...
  async function prepareShareLink() {
    ...
    setShareStatus("Preparing a link…");
    try {
      const overwriteId = state.currentSnapshotLocation === "server" ? state.currentSnapshotId : null;
      const id = await saveSnapshot({
        overwriteId,
        name: document.querySelector("#currentDocName")?.textContent || "Shared board",
        location: "server"
      });
      if (!id) {
        setShareStatus("Draw or write something on the board first, then share.");
        return;
      }
      const url = `${location.origin}${location.pathname}?board=${encodeURIComponent(id)}`;
      if (shareLinkInput) shareLinkInput.value = url;
      setShareStatus("Anyone with this link can open a copy of the board.");
    } catch (error) {
      setShareStatus(error.message || "Could not create a share link.");
    }
  }
  ...
  document.querySelector("#shareCanvasBtn")?.addEventListener("click", () => {
  ...
  document.querySelector("#debugBtn")?.addEventListener("click", (e) => {
    e.preventDefault();
  });
  ...
  aiOrb.addEventListener("pointerdown", (e) => {
    ...
    if (wasOpen) {
      try { aiOrb.setPointerCapture(e.pointerId); } catch {}
    }
  });
  ...
  document.addEventListener("pointerdown", (e) => {
    if (!embodiment.classList.contains("menu-open") || state.radialGesture) return;
    if (embodiment.contains(e.target)) return;
    closeRadialMenu();
  });
  document.querySelectorAll(".radial-action").forEach((button) => {
  ...
  const sharedBoardId = new URLSearchParams(location.search).get("board");
  if (sharedBoardId) {
    loadSnapshot(sharedBoardId, "server").catch((error) => {
      setStatus(error.message || "Could not open the shared board.");
    });
  }
  fit();
  setNavigating(true);
  ...
  async function rasterizePhoto(cmd) {
    const src = String(cmd?.href || cmd?.url || "").trim();
    if (!src || !(src.startsWith("data:image/") || src.startsWith("https://") || src.startsWith("http://") || src.startsWith("/api/"))) return null;
    let objectUrl = null;
    try {
      const img = await new Promise((resolve, reject) => {
        const picture = new Image();
        picture.onload = () => resolve(picture);
        picture.onerror = () => reject(new Error("photo decode"));
      ...
      console.warn("[ATLAS] photo raster failed:", err);
      return null;
    } finally {
  ...
  function lessonImages() {
    return state.images.filter((img) => String(img.sourceName || "").startsWith("atlas-lesson:"));
  }

  function shiftLessonImagesLeft(newPlace) {
    const previous = lessonImages();
    if (!previous.length || !newPlace) return;
    const keep = previous.slice(-3);
    const drop = previous.filter((img) => !keep.includes(img));
    if (drop.length) state.images = state.images.filter((img) => !drop.includes(img));
    const gap = Math.max(90, newPlace.w * 0.06);
    let x = newPlace.x - gap;
  ...
  async function placeLessonImage(cmd, options = {}) {
    const canvas = cmd?.tool === "place_photo" ? await rasterizePhoto(cmd) : await rasterizeSvgPicture(cmd);
    ...
    if (state.pending) acceptPending({ restoreMode: false });
    recordImagesBefore();
    const keepOthers = options.keepOthers === true || cmd?.keepOthers === true || cmd?.archivePrevious === true;
    const archivePrevious = options.archivePrevious === true || cmd?.archivePrevious === true;
    if (archivePrevious) shiftLessonImagesLeft(place);
    else if (!keepOthers) {
  ...
  function clearLessonImages() {
    const kept = state.images.filter((img) => !String(img.sourceName || "").startsWith("atlas-lesson:"));
    if (kept.length === state.images.length) return false;
    recordImagesBefore();
    state.images = kept;
    state.userRevision++;
    save();
  ...
  window.clearPrimerLessonImages = clearLessonImages;
  ...
  window.LUMI6_CANVAS_ADAPTER = {
    ...
    async captureLessonBoardPng() {
      try {
        if (typeof finalizeCanvasForSnapshot === "function") await finalizeCanvasForSnapshot();
        const canvas = await renderExportCanvas();
        if (!canvas) return this.captureBoardImage();
        const png = canvas.toDataURL("image/png");
        canvas.width = canvas.height = 1;
        return png && png.startsWith("data:image/png") && png.length > 200 ? png : this.captureBoardImage();
      } catch (err) {
        console.warn("[Lumi6] lesson board capture failed:", err);
        return this.captureBoardImage();
      }
    },
    ...
    captureBoardImage() {
      try {
        ...
        if (!ink || ink.w < 4 || ink.h < 4) return null;
        ...
        const maxW = 1600, maxH = 1600, targetMin = 1100;
