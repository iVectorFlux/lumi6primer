...
Fetch only the declared origin with `credentials:"omit"`. The HTML owns its initial fetch and refresh timer. Do not use external assets, navigation, forms, cookies, storage, or secrets. Show loading/error states and the last successful update time. After every render call `window.parent.postMessage({type:"lumi6-widget-updated"}, "*")`.

## One-shot example

User writes `` and points right. Produce one `html_widget` there with the strongest recent event, several runners-up, time, depth, location, and USGS attribution.
