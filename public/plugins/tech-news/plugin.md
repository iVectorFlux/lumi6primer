...
Fetch only the declared origin with `credentials:"omit"`. The HTML owns fetching and its timer. Do not use external assets, navigation, forms, cookies, storage, or secrets. Show loading/error states and last successful update. After every render call `window.parent.postMessage({type:"lumi6-widget-updated"}, "*")`.

## One-shot example

User writes ` AI ` and points below. Produce one `html_widget` below showing recent AI-related story titles, age, source, score/comments, source caveat, and update time.
