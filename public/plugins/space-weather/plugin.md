...
Fetch only the declared origin with `credentials:"omit"`. The HTML owns its initial fetch and timer. Do not use external assets, navigation, forms, cookies, storage, or secrets. Show loading/error states and last successful update. After every render call `window.parent.postMessage({type:"lumi6-widget-updated"}, "*")`.

## One-shot example

User writes `?` and points below. Produce one `html_widget` below showing latest Kp, activity level, trend, caveat, and NOAA attribution.
