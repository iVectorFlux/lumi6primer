...
Fetch only the declared origins with `credentials:"omit"`. The HTML owns its initial fetch and 900-second refresh timer; Lumi6 never proxies or refreshes data. Do not use external assets, navigation, forms, cookies, storage, or secrets. After every success or error render, call `window.parent.postMessage({type:"lumi6-widget-updated"}, "*")` so PNG export receives the current view.

## One-shot example

User writes `` and points to an empty area. Produce one `html_widget` there titled ``; its generated HTML resolves Shanghai, shows current conditions plus five forecast days, refreshes every 900 seconds, and follows all runtime rules above.
