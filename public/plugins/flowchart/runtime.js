// Semantic professional-diagram sources rendered inside the existing widget iframe.
  const DIAGRAM_RUNTIME = (() => {
    const VERSION = "lumi6-diagram-source-v1";
    ...
    function responsiveMermaidSource(value, width, height) {
      ...
      if (!directive || /%%\s*lumi6:fixed-layout\b/i.test(source)) return { source, direction:"" };
      const connectors = source.match(/-->|---|-\.-?>|==>/g)?.length || 0,
        responsive = /%%\s*lumi6:responsive\b/i.test(source) || connectors > 10;
      ...
      if (/%%\s*lumi6:responsive\b/i.test(source))
    ...
    function frameRuntime(config, responsiveMermaidSource) {
      ...
      const notify = () => {
        try { parent.postMessage({ type:"lumi6-widget-updated" }, "*"); } catch {}
      };
      ...
      async function renderMermaid() {
        ...
        const paint = async () => {
            ...
            const rendered = await mermaid.render(`lumi6-${Math.random().toString(36).slice(2)}`, next.source);
...
<body>
  ...
  if (typeof window === "object") window.LUMI6_DIAGRAM_RUNTIME = DIAGRAM_RUNTIME;
