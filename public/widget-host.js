...
(() => {
  ...
  function runtime() {
    const UPDATED = "lumi6-widget-updated",
      DRAG_START = "lumi6-widget-drag-start",
      DRAG_MOVE = "lumi6-widget-drag-move",
      DRAG_END = "lumi6-widget-drag-end",
      TOUCH_START = "lumi6-widget-touch-start",
      TOUCH_MOVE = "lumi6-widget-touch-move",
      TOUCH_END = "lumi6-widget-touch-end",
      PAN_START = "lumi6-widget-pan-start",
      PAN_MOVE = "lumi6-widget-pan-move",
      PAN_END = "lumi6-widget-pan-end",
      WHEEL = "lumi6-widget-wheel",
      MOVE_TOLERANCE_PX = 8,
    ...
    function activateHold(press) {
      ...
      document.documentElement.classList.add("lumi6-widget-dragging");
    ...
    function finishPress(event, cancelled = false) {
      ...
      if (![...presses.values()].some((item) => item.active)) document.documentElement.classList.remove("lumi6-widget-dragging");
    }
    function finishMiddlePan(event, cancelled = false) {
      ...
      middlePan = null;
      document.documentElement.classList.remove("lumi6-widget-dragging");
      event.preventDefault?.();
      event.stopImmediatePropagation?.();
      return true;
    }
    addEventListener("pointerdown", (event) => {
      ...
      if (event.pointerType !== "touch" && !hit) {
        if (!widgetState.selected) parent.postMessage({ type:"lumi6-widget-activate" }, "*");
        return;
      }
      const press = {
        pointerId:event.pointerId,
    ...
    function setRuntimeActive(active) {
      ...
      if (active) {
        document.documentElement.classList.remove("lumi6-widget-paused");
      ...
      document.documentElement.classList.add("lumi6-widget-paused");
    }
    function inlineSvgComputedStyles() {
      ...
      for (const svg of document.querySelectorAll("svg")) {
        ...
        if (background && background !== "transparent" && !/^rgba\([^)]*,\s*0(?:\.0+)?\s*\)$/.test(background) && !/\/\s*0(?:\.0+)?\s*\)$/.test(background)) {
          ...
          rect.setAttribute("data-lumi6-snapshot-background", "");
    ...
    async function snapshot(message) {
      ...
      try {
        ...
        const canvas = await globalThis.html2canvas(document.documentElement, {
          ...
          lumi6DirectRendering:true,
          imageTimeout:10000,
        });
        parent.postMessage({ type:"lumi6-widget-snapshot", requestId:message.requestId, dataUrl:canvas.toDataURL("image/png"), width:canvas.width, height:canvas.height }, "*");
        canvas.width = canvas.height = 1;
      } catch (error) {
        parent.postMessage({ type: "lumi6-widget-snapshot-error", requestId: message.requestId, error: error.message }, "*");
      } finally {
        restoreSvgStyles();
      }
    }
    addEventListener("message", (event) => {
      ...
      if (event.data?.type === "lumi6-widget-snapshot-request") void snapshot(event.data);
      else if (event.data?.type === "lumi6-widget-state" && typeof event.data.selected === "boolean" && typeof event.data.active === "boolean"
  ...
  function snapshotError(requestId, message = "Widget snapshot failed") {
    ...
    parent.postMessage({ type:"lumi6-widget-snapshot-error", requestId, error:message }, parentOrigin);
  }
  ...
  function widgetDocument(html, pluginStyles = "") {
    ...
    parsed.querySelectorAll("script:not([src])").forEach((element) => {
      ...
      element.dataset.lumi6ScopedWindowBindings = "";
    });
    ...
    if (pluginStyles) {
      ...
      pluginStyle.dataset.lumi6PluginStyles = "";
      pluginStyle.textContent = pluginStyles;
      parsed.head.append(pluginStyle);
    }
    const bridgeStyle = parsed.createElement("style");
    bridgeStyle.textContent = "html,body{background:transparent!important;color-scheme:light!important;touch-action:none!important;overscroll-behavior:contain}html.lumi6-widget-dragging,html.lumi6-widget-dragging *{cursor:grabbing!important;user-select:none!important}html.lumi6-widget-paused *,html.lumi6-widget-paused *::before,html.lumi6-widget-paused *::after{animation-play-state:paused!important}";
  ...
  function forwardWidgetState() {
    inner.contentWindow?.postMessage({ type:"lumi6-widget-state", ...widgetState }, "*");
  }
  ...
  function forwardDragMessage(message) {
    ...
    if (message.type === "lumi6-widget-drag-start") {
    ...
    if (message.type === "lumi6-widget-drag-move") {
  ...
  addEventListener("message", (event) => {
    ...
    if (event.source === parent && event.origin === parentOrigin) {
      if (message?.type === "lumi6-widget-init") {
      ...
      } else if (message?.type === "lumi6-widget-state" && typeof message.selected === "boolean" && typeof message.active === "boolean"
        ...
        forwardWidgetState();
      } else if (message?.type === "lumi6-widget-snapshot-request" && initialized) {
    ...
    if (message.type === "lumi6-widget-updated") {
      ...
      lastUpdate = now;
      parent.postMessage({ type: "lumi6-widget-updated" }, parentOrigin);
    } else if (message.type === "lumi6-widget-snapshot" && pendingSnapshots.has(message.requestId)) {
    ...
    } else if (message.type === "lumi6-widget-snapshot-error" && pendingSnapshots.has(message.requestId)) snapshotError(message.requestId, "Widget content could not be rendered");
    else if (message.type === "lumi6-widget-activate") parent.postMessage({ type:message.type }, parentOrigin);
  ...
  parent.postMessage({ type: "lumi6-widget-host-ready" }, parentOrigin);
})();
