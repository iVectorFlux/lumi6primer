...
async function createRichUi(options = {}) {
  ...
  return {
    ...
    header(title, breadcrumb, detail = "") {
      ...
      output.write(`${ansi("1;36", "Lumi6 Configuration")}  ${ansi("2", breadcrumb)}\n`);
...
async function chooseModel(ui, provider, configuration) {
  ...
  const choices = provider === PROVIDERS.kimi
    ? [
        ...
        ...(current ? [{ name:`Current Lumi6 model: ${current}`, value:current }] : []),
    ...
    ? [
        ...
        { name:"gpt-5.6-sol (recommended)", value:"gpt-5.6-sol", description:"Best current choice for Lumi6." },
    ...
    : [
        ...
        { name:"claude-opus-4-6", value:"claude-opus-4-6", description:"Supported, but Lumi6 quality may be lower." },
        { name:"Sonnet alias", value:"sonnet", description:"Supported, but Lumi6 quality may be lower." },
...
async function configureSettings(ui, configuration, options) {
  ...
  const legacyTimeout = cleanText(env.CODEX_CLI_TIMEOUT_SECONDS || env.CLAUDE_CLI_TIMEOUT_SECONDS),
    ...
    currentImageFormat = ["webp","png"].includes(cleanText(env.LUMI6_AI_IMAGE_FORMAT).toLowerCase()) ? cleanText(env.LUMI6_AI_IMAGE_FORMAT).toLowerCase() : "webp",
    ...
    trace = await ui.confirm("Record complete AI request details", /^(?:1|true|yes|on)$/i.test(cleanText(env.LUMI6_REQUEST_TRACE))),
  ...
  await options.save({
    ...
    LUMI6_REQUEST_TRACE:trace ? "true" : "false",
    LUMI6_REQUEST_TRACE_LIMIT:traceLimit,
    HOST:host,
    PORT:port,
