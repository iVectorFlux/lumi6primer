...
const { anthropicEffortParameters, normalizedApiEffort, openAiTokenLimit, resolveApiConfig } = require("../server/api-config.js");
...
function resolveConfiguration(args, options = {}) {
  const sourceEnv = options.env || process.env,
    ...
    defaultStateDir = sourceEnv.LUMI6_STATE_DIR ? path.resolve(cwd, sourceEnv.LUMI6_STATE_DIR) : path.join(home, ".lumi6"),
    configFile = args.config ? path.resolve(cwd, args.config) : path.join(defaultStateDir, "config.env"),
    fileEnv = loadEnvFile(configFile),
    configuredStateDir = sourceEnv.LUMI6_STATE_DIR || fileEnv.LUMI6_STATE_DIR,
...
function apiConfigurationIssues(env) {
  ...
  if (env.LUMI6_AI_IMAGE_FORMAT && !["webp", "png"].includes(String(env.LUMI6_AI_IMAGE_FORMAT).trim().toLowerCase())) issues.push("LUMI6_AI_IMAGE_FORMAT");
...
async function testApiConnection(env, options = {}) {
  ...
  const makeRequest = (includeReasoningEffort = true) => {
    ...
    const isReasoningModel = /^(o1|o3)/i.test(String(model || "").trim());
    const validReasoningEfforts = new Set(["low", "medium", "high"]);
    const normEffort = String(effort || "").toLowerCase();
    const shouldAddReasoning = includeReasoningEffort && isReasoningModel && validReasoningEfforts.has(normEffort);

    const bodyObj = {
      model,
      messages: [{ role: "user", content: "Reply with OK." }],
      ...openAiTokenLimit(model, 10)
    };
    if (shouldAddReasoning) {
      bodyObj.reasoning_effort = normEffort;
    }
  ...
  let response;
  try {
    let reqPayload = makeRequest(true);
    response = await fetchImpl(api.endpoint, { method:"POST", redirect:"error", signal:controller.signal, ...reqPayload });
    if (!response.ok) {
      const errorText = await response.text();
      if (errorText.includes("reasoning_effort")) {
        reqPayload = makeRequest(false);
        response = await fetchImpl(api.endpoint, { method:"POST", redirect:"error", signal:controller.signal, ...reqPayload });
      } else {
        const diagnostic = safeApiDiagnostic(errorText, key);
    ...
    if (error.message.includes("API returned HTTP")) throw error;
...
function saveConfiguration(configuration, updates = {}) {
  ...
  const defaults = {
    ...
    LUMI6_AI_IMAGE_FORMAT:String(configuration.env.LUMI6_AI_IMAGE_FORMAT || "webp"),
    ...
    LUMI6_REQUEST_TRACE:String(configuration.env.LUMI6_REQUEST_TRACE || "false"),
    LUMI6_REQUEST_TRACE_LIMIT:String(configuration.env.LUMI6_REQUEST_TRACE_LIMIT || "100"),
  };
...
function configuredTestImage(env) {
  return String(env.LUMI6_AI_IMAGE_FORMAT || "webp").trim().toLowerCase() === "png" ? CONFIG_TEST_IMAGE : CONFIG_TEST_WEBP;
}
...
async function testConfiguredProvider(configuration, options = {}) {
  ...
  try {
    await (options.claudeCaller || callClaudeCli)({
      ...
      systemPrompt:"You are running a Lumi6 configuration test. Do not use tools. Reply with OK only.",
...
async function runDoctor(args, configuration, options = {}) {
  ...
  report(missingAssets.length === 0, missingAssets.length ? `Missing Lumi6 assets: ${missingAssets.join(", ")}` : "Lumi6 assets are present");
  ...
  if (configuration.provider === "codex-cli") report(true, `Model is ${configuration.env.CODEX_CLI_MODEL || "the Codex CLI default for Lumi6's isolated session"}`);
  ...
  if (!configuration.provider) {
    ...
    report(issues.length === 0, issues.length ? `API configuration is incomplete: ${issues.join(", ")}. Run \`lumi6 configure\`.` : "API configuration is ready (no paid request was made)");
...
function schedulePostStartUpdate(server, argv, options, output, errorOutput) {
  const task = () => runPostStartUpdate(server, argv, options, output, errorOutput).catch(error => {
    errorOutput.write(`Lumi6 update check failed: ${error.message}\n`);
...
function helpText() {
  return `Lumi6 ${PACKAGE_JSON.version}\n\nUsage:\n  lumi6 [--config FILE] [--port 3888]\n  lumi6 configure [--config FILE]\n  lumi6 doctor [--api|--kimi|--codex|--claude] [--config FILE]\n  lumi6 --kimi [--model MODEL] [--effort LEVEL]\n  lumi6 --codex [--model MODEL] [--effort LEVEL]\n  lumi6 --claude [--model MODEL] [--effort LEVEL]\n\nOptions:\n  --config <file>   Use this configuration file instead of ~/.lumi6/config.env\n  --api             Use an OpenAI-compatible or Anthropic-compatible API\n  --kimi            Use the authenticated Kimi Code CLI\n  --codex           Use the authenticated Codex CLI\n  --claude          Use the authenticated Claude CLI\n  --model <model>   Override the model for a CLI mode\n  --effort <level>  Override reasoning effort with a known or CLI-supported value\n  --port <port>     Override the configured listening port\n  -h, --help        Show help\n  -v, --version     Show version\n\nRun \`lumi6 configure\` for the interactive configuration center. Known effort values include none, low, medium, high, xhigh, and max; other strings are passed through.\n\nKimi Code CLI installation (run these yourself):\n  macOS/Linux: curl -fsSL https://code.kimi.com/kimi-code/install.sh | bash\n  Windows PowerShell: irm https://code.kimi.com/kimi-code/install.ps1 | iex\n  Then: kimi --version && kimi login\n  Official guide: https://github.com/MoonshotAI/kimi-code\n\nExamples:\n  lumi6 configure\n  lumi6\n  lumi6 --config ./team.env\n  lumi6 --kimi\n  lumi6 --codex --model gpt-5.6-sol --effort xhigh\n`;
}
...
async function main(argv = process.argv.slice(2), options = {}) {
  ...
  catch (error) { errorOutput.write(`Lumi6: ${error.message}\nRun \`lumi6 --help\` for usage.\n`); return 1; }
  if (args.help) { output.write(helpText()); return 0; }
  if (args.version) { output.write(`${PACKAGE_JSON.version}\n`); return 0; }
  if (args.command === "start") output.write(`Lumi6 v${PACKAGE_JSON.version}\n`);
  let configuration;
  try { configuration = resolveConfiguration(args, options); }
  catch (error) { errorOutput.write(`Lumi6 configuration error: ${error.message}\n`); return 1; }
  ...
  const configure = async directProvider => {
    try {
      ...
      errorOutput.write(`Lumi6 configuration failed: ${error.message}\n`);
      return false;
    }
  };
  ...
  if (!configuration.configExists && !sourceConfigured) {
    ...
    if (!interactive) {
      errorOutput.write(`Lumi6 is not configured. Run \`lumi6 configure${args.config ? ` --config ${args.config}` : ""}\` in a terminal first.\n`);
      return 1;
    }
    output.write(`Lumi6 has no saved configuration. Opening the configuration center at ${configuration.configFile}.\n`);
    if (!await configure("")) return 1;
  }
  if (!configuration.provider) {
    errorOutput.write(`Lumi6 has no LLM source. Run \`lumi6 configure\` and select Kimi CLI, Claude CLI, Codex CLI, or API.\n`);
    return 1;
  }
  if (configuration.provider === "api") {
    const issues = apiConfigurationIssues(configuration.env);
    if (issues.length) {
      errorOutput.write(`Lumi6 API configuration is incomplete: ${issues.join(", ")}.\nRun \`lumi6 configure\` to correct it.\n`);
      return 1;
    }
  } else if (configuration.provider === "kimi-cli") {
    const kimi = await runKimiPreflight(configuration, { runner:options.runner });
    if (!kimi.ok) {
      errorOutput.write(`Lumi6 Kimi check failed: ${kimi.error}\nRun \`lumi6 doctor --kimi\` for full diagnostics.\n`);
      return 1;
    }
    output.write(`Lumi6 is using Kimi CLI (${kimi.version}).\nIf Canvas requests cannot reach Kimi, verify or install the CLI yourself:\n  macOS/Linux: curl -fsSL https://code.kimi.com/kimi-code/install.sh | bash\n  Windows PowerShell: irm https://code.kimi.com/kimi-code/install.ps1 | iex\n  Verify: kimi --version\n  Authenticate: kimi login\n  Official guide: https://github.com/MoonshotAI/kimi-code\n`);
  } else if (configuration.provider === "codex-cli") {
    const codex = await runCodexPreflight(configuration, { runner: options.runner });
    if (!codex.ok) {
      errorOutput.write(`Lumi6 Codex check failed: ${codex.error}\nRun \`lumi6 doctor --codex\` for full diagnostics.\n`);
      return 1;
    }
  } else {
    const claude = await runClaudePreflight(configuration, { runner: options.runner });
    if (!claude.ok) {
      errorOutput.write(`Lumi6 Claude check failed: ${claude.error}\nRun \`lumi6 doctor --claude\` for full diagnostics.\n`);
      return 1;
    }
  }
