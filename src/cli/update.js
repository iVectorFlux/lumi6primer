...
const UPDATE_SKIP_ENV = "LUMI6_SKIP_UPDATE_CHECK";
...
function compareVersions(left, right) {
  ...
  if (!a || !b) throw new Error("Lumi6 received an invalid npm version.");
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    try {
      const response = await fetchImpl(url, {
        headers:{ Accept:"application/json", "User-Agent":`lumi6/${PACKAGE_JSON.version}` },
        redirect:"error",
        signal:controller.signal,
      });
      if (!response.ok) throw new Error(`npm registry returned HTTP ${response.status}.`);
      const latest = String((await response.json())?.version || "").trim();
      if (!parsedVersion(latest)) throw new Error("npm registry returned an invalid Lumi6 version.");
...
async function maybeUpdateOnStart(argv, options = {}) {
  ...
  output.write("Checking latest Lumi6 version...\n");
  let latest;
  try {
    latest = await checker();
    if (compareVersions(latest, PACKAGE_JSON.version) <= 0) {
      output.write(`Lumi6 v${PACKAGE_JSON.version} is the latest version.\n`);
      return { checked:true, latest, restarted:false };
    }
  } catch (error) {
    output.write(`Latest Lumi6 version check unavailable (${updateCheckErrorMessage(error)}); continuing with the running service.\n`);
    return { checked:false, restarted:false };
  }

  output.write(`A newer Lumi6 version is available: v${latest} (current v${PACKAGE_JSON.version}).\n`);
  if (!await confirmUpdate("Update Lumi6 now?", options)) {
    output.write(`Continuing with Lumi6 v${PACKAGE_JSON.version}.\n`);
    return { checked:true, latest, restarted:false };
  }

  output.write(`Updating Lumi6 to v${latest}...\n`);
  ...
  try {
    ...
    errorOutput.write(`Lumi6 update failed: ${error.message}\n`);
  }
  if (!installed) {
  ...
  output.write(`Lumi6 v${latest} installed successfully.\n`);
  try {
    if (typeof options.updateFinalizer === "function") await options.updateFinalizer();
  } catch (error) {
    errorOutput.write(`Lumi6 was updated, but the current service could not stop: ${error.message}\nStop it manually, then run \`lumi6\` to start v${latest}.\n`);
    return { checked:true, latest, updated:true, restarted:false, exitCode:1 };
  }
  output.write(`Update complete. Run \`lumi6\` again to start v${latest}.\n`);
