...
function loadEnvConfig() {
  ...
  const defaultStateDir = process.env.LUMI6_STATE_DIR ? path.resolve(process.env.LUMI6_STATE_DIR) : path.join(home, ".lumi6");
  const candidates = [
    path.join(defaultStateDir, "config.env"),
    path.join(ROOT, ".env"),
...
const STATE_DIRECTORY = process.env.LUMI6_STATE_DIR ? path.resolve(process.env.LUMI6_STATE_DIR) : null;
const PRIVATE_PLUGIN_DIRECTORY = process.env.LUMI6_PRIVATE_PLUGIN_DIR
  ? path.resolve(process.env.LUMI6_PRIVATE_PLUGIN_DIR)
  : STATE_DIRECTORY
...
const LOG_FILE = path.join(LOG_DIR, "lumi6.log");
...
const SHARED_CANVAS_DIRECTORY = STATE_DIRECTORY
  ...
  : path.join(os.homedir(), ".lumi6", "canvases", "shared");
...
// These Markdown contracts ship with Lumi6. Files created through the local
...
const AI_IMAGE_FORMAT = normalizeAiImageFormat(process.env.LUMI6_AI_IMAGE_FORMAT);
...
const debugArtifactsValue = optionalBoolean(process.env.LUMI6_DEBUG_ARTIFACTS);
const DEBUG_ARTIFACTS = debugArtifactsValue === true;
const requestTraceValue = optionalBoolean(process.env.LUMI6_REQUEST_TRACE),
  REQUEST_TRACE_ENABLED = requestTraceValue === true,
  requestTraceLimitText = process.env.LUMI6_REQUEST_TRACE_LIMIT?.trim(),
...
function providerConfigurationError() {
  ...
  if (AI_IMAGE_FORMAT === "webp" && !sharp) return "WebP image encoding is unavailable. Reinstall Lumi6 so its Sharp dependency is present, or select PNG in Settings.";
  if (debugArtifactsValue === null) return "LUMI6_DEBUG_ARTIFACTS must be true or false when set.";
  if (requestTraceValue === null) return "LUMI6_REQUEST_TRACE must be true or false when set.";
  if (!requestTraceLimitValid) return "LUMI6_REQUEST_TRACE_LIMIT must be an integer between 1 and 1000.";
...
function providerRequest(key, model, text, atlasImage = null, effort = API_EFFORT, literalTypeset = false, animationEnabled = false, pluginsEnabled = false, omitReasoningEffort = false) {
  ...
  if (typeof text === "string" && text.startsWith("{")) {
    try {
      ...
      if (parsedInput.persona === "teacher" && parsedInput.systemPrompt) {
        fastTalk = parsedInput.fastTalk === true;
        pictureOnly = parsedInput.pictureOnly === true;
        teacherMode = true;
        systemContent = parsedInput.systemPrompt;
        const history = Array.isArray(parsedInput.conversationHistory) ? parsedInput.conversationHistory : [];
        const recent = history.slice(-8).map((turn) => `${turn.role === "student" ? "Student" : "Tutor"}: ${turn.content}`).join("\n");
        const boardRead = parsedInput.boardRead === true;
        const boardNote = atlasImage
          ? boardRead
            ? "The attached photo is the child's whiteboard. Transcribe handwritten math exactly, line by line. ×, *, or a small x between digits is multiply, not the letter x. ÷ and / are divide. Do not compute. Do not invent digits. Ignore printed tutor notes, grid lines, and Lumi6 pictures. If there is no math, expressions must be []."
            : "The attached photo is the child's whiteboard. Transcribe every handwritten mark before you answer. Math: + is plus; ×, *, or a small x between digits is multiply, not the letter x; ÷ and / are divide; = is equals. Compute any arithmetic yourself before you speak. Never guess. Never say the photo is blank if any ink is visible. Ignore printed tutor notes and grid lines."
          : "Answer the student's current question. Do not mention the whiteboard.";
        userText = [boardNote, recent ? `Recent conversation:\n${recent}` : "", `Student's current question: "${parsedInput.studentQuery || ""}"`]
          .filter(Boolean)
          .join("\n\n");
        userText += pictureOnly
          ? "\n\nReply with compact JSON only: {\"picture\":{\"title\":\"...\",\"bg\":\"#eef2ff\",\"parts\":[...]}}."
          : boardRead
  ...
  if (API.format === "anthropic") {
    ...
    const content = atlasImage
      ? [
          { type: "text", text: teacherMode ? userText : text },
  ...
  const isReasoningModel = !isGemini && /^(o1|o3)/i.test(modelLower);
  const validReasoningEfforts = new Set(["low", "medium", "high"]);
  const normEffort = String(effort || "").toLowerCase();
  ...
  return {
    ...
    body: JSON.stringify({
      model,
      ...reasoningParam,
      ...openAiTokenLimit(model, maxOut),
      ...(teacherMode && atlasImage ? { temperature: 0 } : {}),
      ...(teacherMode || (atlasImage && !teacherMode) ? { response_format: { type: "json_object" } } : {}),
      messages
    }),
  };
}
...
Handwriting requires deliberate character-by-character inspection. Inspect stroke groups, spacing, punctuation, and neighboring semantic constraints before deciding each character. Distinguish visually similar characters instead of guessing from a single stroke, and use the magnified focusInset whenever available. Do not let interface language or older context replace pixel evidence. If a character or word remains ambiguous, resolve it from the full phrase and question structure rather than silently changing the sentence topic. Handwritten math and diagrams need extra care: a 3-D cube is a box with faces and depth, not a plus or multiplication sign; a plus/cross is two crossing strokes, not a cube; × or a centered · is multiplication, not the variable x unless neighboring letters make a word or algebra variable; a raised ³ is cubed, a raised ² is squared; a decimal point sits on the baseline between digits (3.14), while a multiplication dot is vertically centered — never drop, invent, or shift decimal points.

Interpret spatial editing gestures as instructions, not ordinary sentence text. A hand-drawn box or circle selects/references the content inside it. An arrow connects the selected source to a destination. Labels near the arrow such as "more", "detail", "expand", "explain", or "why" request a fuller explanation of the selected content; they should not be copied into the response. Respond in the language of the newest substantive user content. If the newest input is only a spatial control label such as "more", follow the language of the selected or referenced content. Preserve intentional mixed-language terminology when useful. Never choose a response language from the interface language alone. Follow an arrow chain to its final arrowhead and place the explanation in the clear space immediately beyond that final arrowhead.
...
function canonicalSharedCanvas(value) {
  ...
  const createdAt=Number(value.createdAt),name=typeof value.name==="string"?value.name.trim().slice(0,48):"",
    theme=["arcane","scifi","research","studio","indic","canvas-pro"].includes(value.theme)?value.theme:"studio",
...
function saveSharedCanvas(value, overwriteId = null) {
  ...
  if(!exists&&sharedCanvasFiles().length>=MAX_SHARED_CANVASES)throw Object.assign(new Error(`The Lumi6 server can retain up to ${MAX_SHARED_CANVASES} shared canvases.`),{status:409});
...
function imageDataUrlParts(dataUrl) {
  const match=/^data:(image\/(?:png|webp|jpe?g));base64,([A-Za-z0-9+/]+={0,2})$/i.exec(String(dataUrl||""));
  if(!match)return null;
  const mimeType=match[1].toLowerCase().replace("image/jpg","image/jpeg"),base64=match[2],buffer=Buffer.from(base64,"base64"),extension=mimeType==="image/webp"?"webp":mimeType==="image/jpeg"?"jpg":"png";
...
async function prepareOutboundAtlas(atlasImage) {
  ...
  if(!sharp)throw new Error("WebP image encoding is unavailable. Select PNG in Settings or reinstall Lumi6.");
  try {
...
function hasAiSession(req) {
  const header = req.headers["x-lumi6-session"];
...
function browserRequestError(req) {
  ...
  if (!expectedOrigin) return "AI requests require the configured Lumi6 host.";
  let origin;
  try { origin = new URL(originText); } catch { return "AI requests require the Lumi6 page origin."; }
  ...
  if (!sameOrigin || origin.username || origin.password || origin.pathname !== "/" || origin.search || origin.hash) return "AI requests require the Lumi6 page origin.";
  if ((localAccessMode !== "open" && !hasAiSession(req)) && !isLoopback(req.socket.remoteAddress)) return "Lumi6 access has expired. Refresh the page and unlock it again.";
  return null;
}
function sharedCanvasReadError(req) {
  const host=requestHost(req),expectedOrigin=canonicalRequestOrigin(req);
  if(!host||!expectedOrigin)return"Canvas storage requires the configured Lumi6 host.";
  if((localAccessMode!=="open"&&!hasAiSession(req))&&!isLoopback(req.socket.remoteAddress))return"Lumi6 access has expired. Refresh the page and unlock it again.";
  return null;
}
function localAccessRequestError(req, requireOrigin = false) {
  const host=requestHost(req),expectedOrigin=canonicalRequestOrigin(req);
  if(!expectedOrigin||!isLanClient(req.socket.remoteAddress)||!isAllowedCliHost(host?.hostname))return"This Lumi6 server is not available from this address.";
  if(!requireOrigin)return null;
  const originText=typeof req.headers.origin==="string"?req.headers.origin.trim():"";
  let origin;
  try { origin=new URL(originText); } catch { return"Refresh this Lumi6 page and try again."; }
  if(origin.origin!==expectedOrigin.origin||origin.username||origin.password||origin.pathname!=="/"||origin.search||origin.hash)return"Refresh this Lumi6 page and try again.";
  return null;
}
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!response.ok) {
          const retryText = response.status === 400 ? responseText : await response.text();
          const errorText = short(retryText, 400);
          const error = new Error(`Model request failed (${response.status}): ${errorText}`);
          error.status = response.status;
          error.upstream = { status: response.status, body: retryText.slice(0, 65536), headers: upstreamResponseTrace(response, null).headers };
          throw error;
        }
      }
...
function pluginAuthoringProviderRequest(key, model, prompt, effort) {
  ...
  const pluginModelLower = String(model || "").toLowerCase();
  const isPluginReasoningModel = !pluginModelLower.includes("gemini") && /^o\d/.test(pluginModelLower);
  return {
    headers:{ "Content-Type":"application/json", Authorization:`Bearer ${key}` },
    body:JSON.stringify({ model, ...openAiTokenLimit(model, 5000), ...(isPluginReasoningModel && effort ? { reasoning_effort:effort } : {}), messages:[{ role:"system", content:PLUGIN_AUTHORING_SYSTEM }, { role:"user", content:prompt }] }),
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/config.js") {
    ...
    return send(res,200,`window.LUMI6_CONFIG=${JSON.stringify(config)};`,"application/javascript; charset=utf-8");
  }
  ...
  if(url.pathname==="/api/canvases"||sharedCanvasMatch) {
    try {
      ...
      return send(res,status,{error:error?.message||"Unable to access the Lumi6 server canvas."});
    }
  }
...
if (startupConfigurationError) {
  console.error(`Lumi6 configuration error: ${startupConfigurationError}`);
