...
function usesOpenAiMaxCompletionTokens(model) {
  ...
  return /^(gpt-5|o1|o3|o4|codex-)/.test(name) || name.includes("gpt-5");
}

function openAiTokenLimit(model, limit) {
  return usesOpenAiMaxCompletionTokens(model)
    ? { max_completion_tokens: limit }
    : { max_tokens: limit };
}

module.exports = { anthropicEffortParameters, anthropicResponseMaxTokens, normalizedApiEffort, openAiTokenLimit, resolveApiConfig, usesOpenAiMaxCompletionTokens };
