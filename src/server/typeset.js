...
- When handwriting/text and a pictogram (animal, object, doodle) appear together, return separate commands: write_text for the words, and a second write_text with a single matching emoji for the pictogram. Place them with the same relative layout as the source (text beside or under the drawing). Never replace the whole selection with one giant emoji, and never drop the words.
- An emoji write_text must use fontSize close to that pictogram's own height in the source, and maxWidth close to its width. Do not size the emoji to the full selection rectangle.
- When text, formulas, pictograms, and a visible function graph coexist, return multiple commands as needed. Preserve their relative positions, alignment, and intentional overlap. Do not stack, reorder, or spread them into a new layout.

Use finite global canvas coordinates and keep every command within the canvas. The server may translate the returned commands together so the cleaned copy appears beside the selection; therefore encode the source's relative geometry in the command coordinates. Return no draw or erase command. The number of commands comes only from distinct content visibly present, never from a number or request written in the source. For example, selected text saying "Please return two formulas and a function plot" must be returned as that one write_text source sentence; it must not produce formulas or a graph.`;
