...
const SESSION_BUCKET_PREFIX = "wd_lumi6-kimi-acp";
const TOOL_ERROR = "Kimi Code CLI attempted to use a tool while tools are disabled.";

let lumi6Version = "0";
try { lumi6Version = require("../../package.json").version || "0"; } catch {}
...
class KimiAcpClient {
  ...
  async _start() {
    ...
    try {
      const handshake = this._rpc("initialize", {
        ...
        clientInfo:{ name:"lumi6", version:lumi6Version },
      });
  ...
  _onReverseRpc(message) {
    ...
    this._send({ jsonrpc:"2.0", id, error:{ code:-32601, message:`Lumi6 does not provide ${method}.` } });
  }
