...
const KIMI_AGENT_FILE = "lumi6-canvas-agent.md";
const KIMI_AGENT_DEFINITION = `---
name: lumi6-canvas
description: Isolated response generator for Lumi6 Canvas
tools: []
subagents: []
---

You are an isolated response generator for Lumi6 Canvas. Follow the user prompt exactly and return only the requested response. You have no tools and must not attempt to read files, run commands, access the network, delegate work, or modify the environment.
`;
...
async function callKimiCliSpawn({ executable = "kimi", model = null, effort = null, prompt, atlasImage = null, signal, env = process.env }) {
  const workDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "lumi6-kimi-"));
...
function kimiAcpWorkDir() {
  ...
  return path.join(os.tmpdir(), `lumi6-kimi-acp-work${uid}`);
}
