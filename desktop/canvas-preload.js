...
const CHANNEL = "lumi6:update-state";
...
const updateApi = Object.freeze({
  getState:() => invoke("lumi6:get-update-state"),
  check:() => invoke("lumi6:update-check"),
  download:() => invoke("lumi6:update-download"),
  dismiss:() => invoke("lumi6:update-dismiss"),
  install:() => invoke("lumi6:update-install"),
...
contextBridge.exposeInMainWorld("lumi6DesktopUpdate", updateApi);
...
function installWindowsBanner() {
  ...
  function render(state) {
    ...
    document.body.classList.toggle("lumi6-desktop-update-visible", visible);
    if (!visible) return;
    ...
    if (state.status === "available") {
      ...
      title.textContent = `Lumi6${version} is ready`;
      detail.textContent = "Install the downloaded update and restart Lumi6.";
      primaryButton.textContent = "Install";
    } else if (state.status === "installing") {
      title.textContent = `Installing Lumi6${version}...`;
      detail.textContent = "Lumi6 will restart when installation finishes.";
      primaryButton.hidden = true;
      closeButton.hidden = true;
    } else if (state.status === "checking") {
      title.textContent = "Checking for Lumi6 updates...";
      primaryButton.hidden = true;
      notesButton.hidden = true;
    } else if (state.status === "up-to-date") {
      title.textContent = `Lumi6 v${state.currentVersion} is up to date`;
      primaryButton.hidden = true;
      notesButton.hidden = true;
    } else {
      title.textContent = "Lumi6 update failed";
