...
const ROOT = path.resolve(__dirname, ".."),
  ...
  WINDOW_ICON = path.join(ROOT, "build", "icons", "lumi6.png"),
  HELP_URL = "https://github.com/iVectorFlux/lumi6primer#quick-start",
...
function loadConfiguration() {
  ...
  configuration.env.LUMI6_STATE_DIR = paths.stateDir;
  configuration.env.LUMI6_PRIVATE_PLUGIN_DIR = paths.privatePlugins;
...
function applyEnvironment(configuration) {
  ...
  const paths = userPaths();
  process.env.LUMI6_STATE_DIR = paths.stateDir;
  process.env.LUMI6_PRIVATE_PLUGIN_DIR = paths.privatePlugins;
...
async function showLanAccessNotice(window) {
  ...
  const result = await dialog.showMessageBox(window, {
    type:"info",
    title:"Lumi6 on your local network",
    message:"Lumi6 is available to devices on your local network.",
    detail:`Open this address on another device:\n\n${currentLanUrls.join("\n")}\n\nKeep Lumi6 open, and use this only on a trusted network.`,
...
function startServer(configuration) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => fail(new Error("Lumi6 server did not become ready.")), 10000);
    try {
...
function macUpdateMenu(state) {
  ...
  if (state.status === "available") {
    return [{
      ...
      submenu:[
        { label:`Lumi6${version} is available`, enabled:false },
        ...noteItems,
  ...
  if (state.status === "downloading") {
    return [{
      ...
      submenu:[
        { label:`Downloading Lumi6${version}...`, enabled:false },
  ...
  if (state.status === "ready") {
    return [{
      ...
      submenu:[
        { label:`Lumi6${version} is ready`, enabled:false },
        ...noteItems,
        ...(noteItems.length ? [{ type:"separator" }] : []),
...
function sendUpdateState(window) {
  ...
  window.webContents.send("lumi6:update-state", updateManager.getState());
}
...
function registerIpc() {
  ...
  ipcMain.handle("lumi6:get-update-state", event => fromCanvas(event) ? updateManager?.getState() : null);
  ipcMain.handle("lumi6:update-check", event => fromCanvas(event) ? updateManager?.check(true) : false);
  ipcMain.handle("lumi6:update-download", event => fromCanvas(event) ? updateManager?.download() : false);
  ipcMain.handle("lumi6:update-dismiss", event => fromCanvas(event) ? updateManager?.dismiss() : false);
  ipcMain.handle("lumi6:update-install", event => fromCanvas(event) ? updateManager?.install() : false);
  ipcMain.handle("lumi6:get-settings", () => {
  ...
  ipcMain.handle("lumi6:copy-text", (_event, input) => {
    const text = String(input ?? "");
    if (!text || text.length > 4096 || /[\r\n\0]/.test(text)) return { ok:false };
    clipboard.writeText(text);
    return { ok:true };
  });
  ipcMain.handle("lumi6:install-cli", async (_event, provider) => {
  ...
  ipcMain.handle("lumi6:save-and-test", async (_event, input) => {
    try {
      ...
      loaded.configuration.env.LUMI6_STATE_DIR = loaded.paths.stateDir;
      loaded.configuration.env.LUMI6_PRIVATE_PLUGIN_DIR = loaded.paths.privatePlugins;
      ...
      try {
        ...
        diagnostic = await Promise.race([
          ...
          new Promise((_, reject) => {
            timer = setTimeout(() => {
              ...
              error.code = "LUMI6_SETTINGS_TEST_TIMEOUT";
              reject(error);
        ...
        return {
          ok:false,
          saved:true,
          timedOut:error.code === "LUMI6_SETTINGS_TEST_TIMEOUT",
  ...
  ipcMain.handle("lumi6:open-help", () => shell.openExternal(HELP_URL));
}
...
async function bootstrap() {
  ...
  try {
    ...
    await dialog.showMessageBox({
      type:"error",
      title:"Lumi6 could not start",
      message:"Lumi6 could not start its local canvas service.",
...
if (gotLock) {
  ...
  app.whenReady().then(bootstrap).catch(error => {
    void dialog.showErrorBox("Lumi6 startup failed", error.message || String(error));
    app.quit();
  });
  app.on("activate", () => {
