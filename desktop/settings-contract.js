...
function kimiPresetUpdates(configuration) {
  const env = configuration?.env || {},
    desktopProvider = String(env.LUMI6_DESKTOP_PROVIDER || "").toLowerCase();
  if (configuration?.provider !== "api" || desktopProvider !== "kimi") return {};
  const requestedProduct = String(env.LUMI6_KIMI_PRODUCT || "code"),
    requestedRegion = String(env.LUMI6_KIMI_REGION || "global"),
    ...
    updates = {};
  if (requestedProduct !== product) updates.LUMI6_KIMI_PRODUCT = product;
  if (requestedRegion !== region) updates.LUMI6_KIMI_REGION = region;
...
function normalizeSettings(input, options = {}) {
  ...
  const updates = {
    ...
    LUMI6_DESKTOP_PROVIDER:provider,
    AI_EFFORT:effort,
    AI_TIMEOUT_SECONDS:String(timeout),
    LUMI6_AI_IMAGE_FORMAT:imageFormat,
    AUTO_AI_DELAY_SECONDS:String(autoDelay),
    HOST:host,
    PORT:String(port),
    LUMI6_REQUEST_TRACE:input.requestTrace === true ? "true" : "false",
    LUMI6_REQUEST_TRACE_LIMIT:String(traceLimit),
  ...
  if (provider === "api" || provider === "kimi") {
    ...
    if (provider === "kimi") {
      ...
      Object.assign(updates, { LUMI6_KIMI_PRODUCT:product, LUMI6_KIMI_REGION:region });
    }
...
function publicSettings(configuration, options = {}) {
  const sourceEnv = configuration.env || {},
    ...
    configuredKimiProduct = String(env.LUMI6_KIMI_PRODUCT || "code"),
    configuredKimiRegion = String(env.LUMI6_KIMI_REGION || "global"),
  ...
  return {
    ...
    imageFormat:String(env.LUMI6_AI_IMAGE_FORMAT || "webp"),
    autoDelay:String(env.AUTO_AI_DELAY_SECONDS || "5"),
    host:String(env.HOST || "0.0.0.0"),
    port:String(env.PORT || "3888"),
    requestTrace:/^(?:1|true|yes|on)$/i.test(String(env.LUMI6_REQUEST_TRACE || "false")),
    traceLimit:String(env.LUMI6_REQUEST_TRACE_LIMIT || "100"),
