...
- [x] Stop stretching interactives to fill the screen. The generated artifacts put an absolutely positioned canvas inside `.artifact-viewport`, which flexed to the window height; the embed CSS now locks that viewport to `--lumi6-aspect` (16/9 default) and the page ends after the controls. The embed reports its own content height via `postMessage`, and the board sizes the iframe to it. Verified with `scripts/check-interactive-layout.js`: at 390×844 the canvas is 390×219 with a backing store of exactly 390×219 (no distortion) and the whole interactive is 355–389px, leaving ~460–490px for text.
- [x] Use that freed space: the playground now shows the title with subject/class badges above, and `interactive_idea` ("How to explore") plus `concept` ("What you are learning") below. Empty columns render nothing rather than bare headings.
- [ ] The scenes themselves are authored for ~960px wide and draw at fixed pixel offsets, so at 390px the edge labels clip (see the P-V diagram axis text). Aspect-locking can't fix that — either make the generator draw relative to canvas width, or render at a fixed logical width and scale to fit, which shrinks text to ~40%.
