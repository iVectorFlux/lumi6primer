...
const ICON_SOURCE = path.join(ROOT, "build", "icons", "lumi6-1024.png");
...
async function writeSplash(target) {
  ...
  const temporary = `${target}.lumi6.png`;
  await sharp({
...
function configureIosNetwork() {
  ...
  info.NSLocalNetworkUsageDescription = "Connect to a Lumi6 server on your local network.";
  fs.writeFileSync(plistPath, plist.build(info));
}

function ensurePlatform(platform) {
  fs.copyFileSync(ICON_SOURCE, path.join(MOBILE_ROOT, "web", "lumi6-mark.png"));
...
function copyArtifact(source, extension, suffix) {
  ...
  const target = path.join(RELEASE_DIR, `Lumi6-${APP_PACKAGE.version}-${suffix}.${extension}`);
...
async function buildIos() {
  ...
  configureIosNetwork();
  cap("sync", "ios");
  const archiveRoot = path.join(RELEASE_DIR, "ios", "Lumi6.xcarchive");
  ...
  fs.cpSync(appPath, path.join(payloadRoot, "Lumi6.app"), { recursive:true });
  const ipa = path.join(RELEASE_DIR, `Lumi6-${APP_PACKAGE.version}-ios-unsigned.ipa`);
