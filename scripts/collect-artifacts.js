...
const ROOT = path.resolve(__dirname, ".."), source = path.join(ROOT, "out", "make"), destination = path.join(ROOT, "release"),
  targetPlatform = process.env.LUMI6_TARGET_PLATFORM || process.platform,
  targetArch = process.env.LUMI6_TARGET_ARCH || process.arch,
...
for (const file of walk(source)) {
  ...
  let name = extension === ".dmg" ? `Lumi6-${pkg.version}-mac-${targetArch}.dmg`
    : extension === ".zip" ? `Lumi6-${pkg.version}-${targetPlatform === "darwin" ? "mac" : "win"}-${targetArch}.zip`
...
const icon = path.join(ROOT, "build", "icons", "lumi6.ico");
if (targetPlatform === "win32" && fs.existsSync(icon)) {
  const target = path.join(destination, "lumi6.ico");
